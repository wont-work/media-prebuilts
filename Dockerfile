...
RUN --mount=type=cache,sharing=locked,target=/var/lib/apt \
 ...
 && apt update \
 && apt-get install --no-install-recommends -y meson ninja-build build-essential pkg-config libglib2.0-dev libexpat1-dev libtiff-dev liblcms2-dev libspng-dev libhwy-dev libwebp-dev libopenjp2-7-dev libjxl-dev libjpeg-turbo8-dev \
                                               yasm libdav1d-dev
